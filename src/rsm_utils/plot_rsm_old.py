...
class VTIViewer(QMainWindow):
    def __init__(self):
        ...
        self.add_to_3d_button = QPushButton("Add to 3D")
        slice_control_layout.addWidget(self.add_to_3d_button)
        self.remove_from_3d_button = QPushButton("Remove from 3D")
        slice_control_layout.addWidget(self.remove_from_3d_button)
    ...
    def add_to_3d(self):
        ...
        try:
            ...
            # Remove previous slice on this specific axis representation if it exists
            self.remove_from_3d(axis_char)

            # Generate orthogonal slice mesh using PyVista built-in slice function
            slice_mesh = self.grid.slice(normal=axis_map[axis_char], origin=(0, 0, val) if axis_char=='L' else ((0, val, 0) if axis_char=='K' else (val, 0, 0)))

            # Map the color scale limits from input strings
            vmin = float(self.vmin_input.text())
            vmax = float(self.vmax_input.text())

            # Add to PyVista plotter environment
            ...
            # Store the actor reference linked to its axis ID
            self.slice_actors[axis_char] = actor
            self.plotter.render()

        except Exception as e:
            print("Error adding slice to 3D:", e)

    def remove_from_3d(self, target_axis=None):
        """Removes interactive slices from PyVista scene. Clears all slices if target_axis is omitted."""
        if not self.slice_actors:
            return

        # Determine which keys to drop
        axes_to_remove = [target_axis] if target_axis in self.slice_actors else list(self.slice_actors.keys()) if target_axis is None else []

        for axis in axes_to_remove:
            actor = self.slice_actors.pop(axis, None)
            if actor:
